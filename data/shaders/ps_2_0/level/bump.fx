//
// Diffuse/specular normal mapping shader with adjustable 
// glossiness (diffuse map alpha channel) and adjustable 
// shininess (specular exponent).
//
// Map channels:
// - Basemap: Diffuse color (+ glossiness in alpha)
// - Normal map: Normal + optional component in alpha (see below)
// - Optional glow strength in normal map alpha (ifdef ENABLE_GLOW)
// - Optional transparency in normal map alpha (ifdef ENABLE_TRANSPARENCY)
...
// diffuse map & glossiness in alpha
...
// transparency (in normal map alpha channel)
...
float4 	LIGHTC0 : LIGHTCOLOR <int LightRef = 0;> = { 1.0f, 1.0f, 1.0f, 0.0f };
...
struct PS_IN 
{
	...
	half3x3 tangenttoworld 	: TEXCOORD5;
	#endif
};
...
half4 pshader( PS_IN IN ) : COLOR
{
	...
	half silevel = tex2D(normaltex, uv).w;
	...
	// transparency
	#ifdef ENABLE_TRANSPARENCY
	half alpha = tex2D(normaltex, uv).w;
	#else
	half alpha = 1.0;
	#endif
...
half4 psGlow( PS_IN_GLOW IN ) : COLOR
{
	...
	half weight = tex2D(normaltex, IN.uv1).w;
	return color * weight;
#else
